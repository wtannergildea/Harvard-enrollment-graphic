...
final_join <- left_join(spring_2016, join_02, by = "course_id") %>% 
  ...
  mutate(avg_enrollment =  (u_grad.2016 + u_grad.2017 + u_grad.2018 + u_grad.2019)/4) %>% 
...
gathered <- final_join %>% 
  ...
  select( - avg_enrollment) %>% 
  arrange(course_title)
```

```{r plot}

ggplot(gathered, aes(x = course, y = course_title, size = count, color = course_name)) +  
